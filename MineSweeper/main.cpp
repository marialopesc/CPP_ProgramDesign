...
void initField(int num_of_mine); // done
bool isMine(int i, int j); // done
...
void initField(int num_of_mine) {
    ...
    for (int i = 0; i < height; i++) { // this will go from 0 until 7
        for (int j = 0; j < width; j++) { // this will go from 0 til 4
            // i, j, UNKNOWN;
            setCell(i, j, UNKNOWN ); //setting all the cells as unknown, so they will all be dashes
        }

    }
    setMine(num_of_mine); // calling the setMine function to randomly assign the number of mines in the field (predefined)
...
bool isMine(int i, int j) {
    if ((getCell(i, j) == UNFLAGGED_MINE) || (getCell(i, j) == FLAGGED_MINE)) { // telling that if a cell is unflagged or flagged,
                                                                                // it will return true to the checking if a cell is a mine
        return true;
}   //Complete me // done
    else {
        return false;
...
//Based on the game description, if the cell is not a mine, the user will see the number of mines in the surrounding cells as the value of the selected cell
...
//	None
void reveal(int i,int j) {
    if (getCell(i, j) >= 0 || isMine(i, j)){ // checking if it's inside the boundaries, and checking if there is mine
        return;
    }

    int count = 0;

 // these if statements below will check each of the 8 boxes around the cell. it make sure they are within the bounds of i and j.
    if (isMine(i, j - 1) && (j > 0)) { // checks on left box
        count++;
    }
    if ((isMine(i, j + 1) && (j < width - 1))) { // // checks on right box
        count++;
    }
    if ((isMine(i - 1, j) && (i > 0))) { // checks on below box
        count++;
    }
    if (isMine(i + 1, j) && (i < height - 1)) { // checks on top box
        count++;
    }
    if ((isMine(i - 1, j - 1) && (i > 0) && (j > 0))) { //checks on the diagonal bottom left box
        count++;
    }
    if ((isMine(i + 1, j + 1)) && (i < height - 1) && (j < width - 1)) { //checks on the diagonal top right box
        count++;
    }

    if ((isMine(i + 1, j - 1) && (i < height - 1)) && (j > 0)) { // checks on the diagonal top left box
        count++;
    }
    if ((isMine(i - 1, j + 1) && (i > 0)) && (j < width - 1)) { // checks on the diagonal bottom right box
        count++;
    }
    setCell(i, j, count);


    //Complete me // CHECK IT
    if (count == 0) {
        if (j > 0) {
            reveal(i, j - 1); //doing a recursion on left box
        }

        if (j < width - 1) {
            reveal(i, j + 1);//doing a recursion on right box
        }

        if (i > 0) {
            reveal(i - 1, j); //doing a recursion on the below box
        }

        if (i < height - 1) {
            reveal(i + 1, j); //doing a recursion on the above box
        }

        if ((i > 0) && (j > 0)) {
            reveal(i - 1, j-1); //doing a recursion on the diagonal bottom left box
        }

        if ((i < height - 1) && (j < width - 1)) {
            reveal(i + 1, j + 1); //doing a recursion on the diagonal top right box
        }

        if ((i < height - 1) && (j > 0)) {
            reveal(i + 1, j - 1); //doing a recursion on the diagonal top left box
        }

        if ((i > 0) && (j < width - 1)) {
            reveal(i - 1, j + 1); //doing a recursion on the diagonal bottom right box
        }
...
//	None
void executeCmd(string cmd) {
    int a = cmd.at(1) - 48; // turning the ascii value into the int wanted
    int b = cmd.at(2) - 48;

    if (cmd.at(0) == 'r'){ // checking if the user typed "r" for reveal
        if(isMine(a, b)){
            explode = true; // if it is correct, it'll return true to explode
        }
        else {
            reveal(a,b);
            }
    }
    if (cmd.at(0) == 'f'){ // checking if the user typed "f" for flag
        if (isMine(a, b)) {
            setCell(a, b, FLAGGED_MINE); // if so, it will turn into flagged_mine
            num_correct_flagged_mine++;             // and count up the correct flags
        }
        else {
            setCell(a, b, INCORRECT_FLAGGED_MINE); //otherwise, it'll be set as an incorrect flagged and
            num_incorrect_flagged_mine++;                     // it will count up the incorrect flagged mine
        }
    }


    if (cmd.at(0) == 'u'){ // checking if the user typed "u" for undo
        if (getCell(a, b) == FLAGGED_MINE){ // if it was typed as flagged before, it'll subtract 1 from num_correct_flagged_mine
            num_correct_flagged_mine--;
            setCell(a, b, UNFLAGGED_MINE); // and it will turn from flagged to unflagged mine
        }
        if (getCell(a, b) == INCORRECT_FLAGGED_MINE){ //if it was incorrect flagged, it will subtract 1 from num_incorrect_flagged_mine
            num_incorrect_flagged_mine--;
            setCell(a, b, UNKNOWN);             // and it will switch from incorrect flagged back to unknown
        }
